...
- `/inbox-automation`: an owner-beta Gmail-to-reviewed-actions workflow. The default setup asks for the initial scan count and private owner key, scans recent inbox messages without manual labels, automatically creates a Sheet only when approved Sheet rows need a destination, and progressively discloses query and destination customization.
