...
The `/inbox-automation` preset uses Google Identity Services' browser token model. A short-lived OAuth token is held in React state only and requests Gmail read-only, Sheets, and Calendar event scopes. Gmail list/get requests use metadata format; the browser sends only message ID, sender, subject, timestamp, and a 1,000-character maximum snippet to `/api/classify`. The server function checks `AUTOMATION_ACCESS_KEY`, accepts only JSON requests up to 64 KiB, uses server-only `OPENAI_API_KEY`, calls the Responses API with a strict JSON Schema, and stores no model response. Classifier responses are marked `no-store`. The browser validates that there is exactly one result per source message before displaying drafts. Nothing is written until the user selects the Sheet and/or Calendar action. Sheet rows are deduplicated by Gmail message ID; Calendar events use deterministic IDs and private visibility.

The owner-beta access key stays only in page memory and is cleared on disconnect or refresh. New sessions default to five messages under the dedicated `workflow-lab-test` Gmail label instead of scanning the recent inbox. Vercel applies an enforced Content Security Policy, denies framing, preserves the Google OAuth popup boundary, disables unused browser capabilities, removes cross-site referrers, and prevents MIME sniffing. These preliminary controls reduce accidental exposure and common browser attacks; they do not replace professor authentication, durable per-user authorization, rate limiting, audit logs, or a reviewed production privacy/retention design.
