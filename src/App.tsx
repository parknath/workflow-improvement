...
import { ArrowRight, BookOpen, CalendarDays, Check, ChevronLeft, ChevronRight, Clock3, Download, FileCheck2, Gauge, GraduationCap, Inbox, Layers3, Menu, MoveDown, MoveUp, Play, Plus, RefreshCw, ShieldCheck, Sparkles, Trash2, Workflow, X } from "lucide-react";
...
import { formatListInput, updateListDraft } from "./listInput";
...
type Route = "/" | "/how-it-works" | "/workflows" | "/demo" | "/intake" | "/sample-result" | "/inbox-automation";
const routes: Route[] = ["/", "/how-it-works", "/workflows", "/demo", "/intake", "/sample-result", "/inbox-automation"];
const labels: Record<Route, string> = { "/": "Home", "/how-it-works": "How it works", "/workflows": "Workflows", "/demo": "Demo", "/intake": "Analyze", "/sample-result": "Sample result", "/inbox-automation": "Inbox automation" };
  ...
  return <main className="simple-home">
    <section className="simple-hero">
      ...
      <div className="starter-panel" aria-labelledby="starter-heading">
        ...
        <button className="automation-starter" onClick={() => go("/inbox-automation")}><span className="starter-icon"><Inbox/></span><span><b>Turn important email into actions</b><small>Connect Gmail, classify with ChatGPT, then approve Sheet tasks and Calendar reminders.</small></span><span className="status ready">Owner beta</span><ChevronRight/></button>
        <div className="custom-divider"><span>Need something else?</span></div>
        <button className="custom-starter" onClick={() => openStarter(go, { ...initialIntake, currentSteps: [emptyStep()] }, false)}><span><Sparkles/><span><b>Design a custom guided workflow</b><small>Create instructions and reusable assets for a different process. This path does not connect external apps.</small></span></span><ArrowRight /></button>
...
function ListField({ label, value, onChange, placeholder = "", multiline = false }: { label: string; value: string[]; onChange: (value: string[]) => void; placeholder?: string; multiline?: boolean }) {
  const externalValue = formatListInput(value, multiline);
  const [draft, setDraft] = useState(externalValue);
  const editing = useRef(false);
  useEffect(() => { if (!editing.current) setDraft(externalValue); }, [externalValue]);
  const change = (raw: string) => { const next = updateListDraft(raw); setDraft(next.raw); onChange(next.values); };
  const finish = () => { editing.current = false; setDraft(formatListInput(updateListDraft(draft).values, multiline)); };
  return <label className="field"><span>{label}</span>{multiline ? <textarea value={draft} onFocus={() => { editing.current = true; }} onChange={e => change(e.target.value)} onBlur={finish} placeholder={placeholder}/> : <input value={draft} onFocus={() => { editing.current = true; }} onChange={e => change(e.target.value)} onBlur={finish} placeholder={placeholder}/>}</label>;
}
...
export function App() { const [route,go]=useRoute(); return <div className="app"><Header route={route} go={go}/>{route==="/"&&<Home go={go}/>} {route==="/how-it-works"&&<HowItWorks go={go}/>} {route==="/workflows"&&<Workflows go={go}/>} {route==="/demo"&&<Demo go={go}/>} {route==="/intake"&&<Intake go={go}/>} {route==="/sample-result"&&<SampleResult go={go}/>} {route==="/inbox-automation"&&<InboxAutomation/>}<Footer go={go}/></div>; }
