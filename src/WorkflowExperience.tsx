...
export function GeneratedWorkflowExperience({ workspace, setWorkspace, storageWarning, downloadPackage, downloadIntake, reviseIntake }: {
  ...
  if (mode === "summary" && summarizeCompletedRun(workspace)) return <RunSummary workspace={workspace} startNextRun={start} showWorkflow={() => setMode("overview")}/>;
  ...
  return <main className="result-page generated-result">
    ...
    <section className="section workflow-downloads"><div><span className="eyebrow">Keep a portable copy</span><h2>Downloads remain available.</h2><p>The active workflow, immediately previous approved version, and intake can be saved locally. Browser storage is convenient, not durable account storage.</p></div><div className="generated-actions"><button className="button secondary" onClick={downloadPackage}><Download/> Download active workflow</button>{workspace.previousPackage && <button className="button secondary" onClick={() => downloadJson(JSON.stringify(workspace.previousPackage, null, 2), `${pkg.metadata.workflowName.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"") || "workflow"}-previous-v${workspace.previousPackage?.version}.json`)}><Download/> Download previous v{workspace.previousPackage.version}</button>}<button className="button secondary" onClick={downloadIntake}>Download intake</button><button className="button text" onClick={reviseIntake}>Revise the intake</button></div></section>
  </main>;
}
...
function RunMeasurement({ workspace, setWorkspace, setMode }: { workspace: WorkflowWorkspace; setWorkspace: (workspace: WorkflowWorkspace) => void; setMode: (mode: ExperienceMode) => void }) {
  ...
  const hasBaseline = Boolean(workspace.lastRunSummary);
  ...
  return <main className="workflow-run-page"><section className="measurement-shell"><span className="eyebrow">{hasBaseline ? "Comparable-run measurement" : "First-run measurement"}</span><h1>What actually happened?</h1><p>{hasBaseline ? "Record this run on the same measures so Workflow Lab can show the observed differences from your prior baseline." : "Separate the prototype estimate from your measured result. This creates a baseline for deciding whether the next run is genuinely better."}</p><form className="measurement-form" onSubmit={submit}><div className="two-fields">
  ...
  </div><Field label="What should change before the next run?" value={measurement.nextChange} onChange={(value) => set("nextChange", value)} multiline placeholder="Name the smallest change that would make the next run more useful."/><div className="recorded-run-facts"><span><b>{run.completedStepOrders.length}</b> completed</span><span><b>{run.skippedStepOrders.length}</b> skipped</span><span><b>{run.problems.length}</b> problems reported</span></div>{errors.length > 0 && <div className="form-errors" role="alert"><b>Complete the measurement:</b>{errors.map((error) => <span key={error}>{error}</span>)}</div>}<div className="form-actions"><button className="button secondary" type="button" onClick={() => { setWorkspace({ ...workspace, currentRun: { ...run, status: "running" } }); setMode("run"); }}><ChevronLeft/> Return to workflow</button><button className="button primary" type="submit"><Save/> Save run result</button></div></form>
  </section></main>;
}

function RunSummary({ workspace, startNextRun, showWorkflow }: { workspace: WorkflowWorkspace; startNextRun: () => void; showWorkflow: () => void }) {
  const summary = summarizeCompletedRun(workspace)!;
  const baseline = workspace.lastRunSummary?.runId === summary.runId ? null : workspace.lastRunSummary;
  const comparison = baseline ? compareWorkflowRuns(baseline, summary) : null;
  const [notice, setNotice] = useState("");
  const filename = `${summary.workflowName.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"") || "workflow"}-${comparison ? "run-comparison" : "run-summary"}-v${summary.workflowVersion}.json`;
  const evidence = serializeRunEvidence(summary, baseline);
  const copy = async () => { try { await navigator.clipboard.writeText(evidence); setNotice(comparison ? "Two-run comparison copied." : "First-run summary copied."); } catch { setNotice("Copy was unavailable. Download the evidence instead."); } };
  const signedDelta = (value: number, lowerIsBetter = false) => value === 0 ? "No change" : lowerIsBetter ? `${Math.abs(value)} ${value < 0 ? "fewer" : "more"}` : `${value > 0 ? "+" : ""}${value}`;
  return <main className="workflow-run-page"><section className="summary-shell"><div className="success-icon"><Check/></div><span className="eyebrow">Run recorded · workflow v{summary.workflowVersion}</span><h1>{comparison ? "Your second result can now be compared." : "Your first result is now a baseline."}</h1><p>{comparison ? "These are observed differences between two runs—not proof that Workflow Lab caused the change." : "This is measured evidence from your run, not a claim that Workflow Lab saved time or improved the academic outcome."}</p>{notice && <div className="approval-notice" role="status"><Check/><span>{notice}</span></div>}<div className="summary-metrics"><article><span>Task completed</span><b>{summary.taskCompleted ? "Yes" : "No"}</b></article><article><span>Actual time</span><b>{summary.actualTotalMinutes} min</b></article><article><span>Usefulness</span><b>{summary.usefulnessRating} / 5</b></article><article><span>Corrections</span><b>{summary.correctionCount}</b></article></div>{comparison && <div className="run-comparison" aria-label="Comparison with the prior run"><div><span className="eyebrow">Prior run → current run</span><h2>What changed on the comparable run?</h2><p>Workflow v{baseline?.workflowVersion} → v{summary.workflowVersion}</p></div><div className="comparison-metrics"><article><span>Actual time</span><b>{comparison.timeDeltaMinutes === 0 ? "No change" : `${Math.abs(comparison.timeDeltaMinutes)} min ${comparison.timeDeltaMinutes < 0 ? "less" : "more"}`}</b><small>{baseline?.actualTotalMinutes} → {summary.actualTotalMinutes} min</small></article><article><span>Usefulness</span><b>{signedDelta(comparison.usefulnessDelta)}</b><small>{baseline?.usefulnessRating} → {summary.usefulnessRating} / 5</small></article><article><span>Corrections</span><b>{signedDelta(comparison.correctionDelta, true)}</b><small>{baseline?.correctionCount} → {summary.correctionCount}</small></article><article><span>Completion</span><b>{baseline?.taskCompleted && summary.taskCompleted ? "Both completed" : baseline?.taskCompleted === summary.taskCompleted ? "No change" : "Changed"}</b><small>{baseline?.taskCompleted ? "Yes" : "No"} → {summary.taskCompleted ? "Yes" : "No"}</small></article></div></div>}<div className="summary-detail"><p><b>Most useful asset</b>{summary.mostUsefulAsset}</p><p><b>Least useful step</b>Step {summary.leastUsefulStepOrder}</p><p><b>Reuse intent</b>{summary.reuseIntent}</p><p><b>Next change</b>{summary.nextChange}</p></div><div className="next-run-panel"><RefreshCw/><div><h2>{comparison ? "What should happen next?" : "Why use it again?"}</h2><p>{comparison ? "Use this comparison to decide whether to keep the active workflow, approve another focused correction, or stop. Do not infer improvement from time alone when completion or usefulness worsened." : `The next comparable run tests whether active v${workspace.activePackage.version} is easier to follow, requires fewer corrections, or produces a more reliable result. That improvement loop—not access to a static template—is the intended recurring value.`}</p></div></div><div className="summary-actions"><button className="button primary" onClick={startNextRun}><Play/> Start next run</button><button className="button secondary" onClick={copy}><Copy/> Copy {comparison ? "comparison" : "summary"}</button><button className="button secondary" onClick={() => downloadJson(evidence, filename)}><Download/> Download {comparison ? "comparison" : "summary"}</button><button className="button text" onClick={showWorkflow}>Review active workflow</button></div>
  </section></main>;
}
...
function CorrectionPanel({ workspace, setWorkspace, setMode, setNotice }: { workspace: WorkflowWorkspace; setWorkspace: (workspace: WorkflowWorkspace) => void; setMode: (mode: ExperienceMode) => void; setNotice: (notice: string) => void }) {
  ...
  const approve = () => { if (!draft) return; const approved = approveWorkflowRevision(draft); setWorkspace(applyApprovedPackageForNextRun(workspace, approved.package)); setNotice(`Revision ${approved.package.version} is active for the next run. Previous v${draft.originalVersion} remains saved in this browser.`); setMode("measurement"); };
